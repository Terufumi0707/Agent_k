...
- ブラウザで選択した `.mp3` / `.mp4` は `/minutes/jobs/audio` で backend 側へ自動アップロードされ、保存先パスが `audio_path` としてジョブ作成に使われます。
...
```bash
curl -X POST http://localhost:8000/minutes/jobs/audio \
  -F "file=@./artifacts/meeting.mp3"
```
...
フロントエンドでは `/minutes/jobs/audio` を利用してファイルを直接送信します。`/minutes/jobs` は既存互換として `audio_path` 指定でも実行できます。

```bash
# 推奨: 音声を直接アップロードしてジョブ作成
curl -X POST http://localhost:8000/minutes/jobs/audio \
  -F "file=@./artifacts/meeting.mp3"

# 既存互換: server 内パスを直接指定
