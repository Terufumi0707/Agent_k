...
def test_start_creates_fallback_candidate_when_draft_returns_empty(tmp_path):
    orchestrator, _, draft, _, _ = _build_orchestrator(tmp_path)
    draft.run.return_value = {"candidates": []}

    job = orchestrator.start(InputType.TRANSCRIPT, transcript="会議ログ本文", audio_path=None)

    assert len(job.candidates) == 1
    assert "会議ログ本文" in job.candidates[0].sections["会議概要"]
