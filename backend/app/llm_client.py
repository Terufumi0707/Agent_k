...
def generate_autonomous_question(prompt: str) -> Dict[str, object]:
    api_key = get_gemini_api_key()
    if not api_key:
        return {}
    ...
    try:
        ...
        response.raise_for_status()
        content = response.json()["candidates"][0]["content"]["parts"][0]["text"]
        return normalize_autonomous_output(content)
    except (httpx.HTTPError, KeyError, ValueError, json.JSONDecodeError):
        return {}
...
def normalize_autonomous_output(content: str) -> Dict[str, object]:
    trimmed = content.strip()
    if trimmed.startswith("```"):
        trimmed = re.sub(r"^```(?:json)?", "", trimmed).strip()
        trimmed = re.sub(r"```$", "", trimmed).strip()
    ...
    if raw.get("a_number"):
        output["a_number"] = raw.get("a_number")
    if isinstance(raw.get("work_types"), list):
        output["work_types"] = raw.get("work_types")
    if raw.get("date"):
        output["date"] = raw.get("date")
    if raw.get("date_inferred") is not None:
        output["date_inferred"] = raw.get("date_inferred")
    if raw.get("question"):
        output["question"] = raw.get("question")
    return output
