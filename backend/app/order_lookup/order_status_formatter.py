...
from app.llm_client import generate_with_system_and_user, generate_with_system_and_user_async
...
class OrderStatusFormatter:
    def __init__(
        ...
        llm_client_async: Callable[[str, str], object] = generate_with_system_and_user_async,
    ) -> None:
    ...
    async def format_async(self, query_result_json: str) -> str:
        user_prompt = f"query_result_json:\n{query_result_json}"
        message = await self._llm_client_async(system_prompt=self._system_prompt, user_prompt=user_prompt)
        if message:
            return message
        return "照会結果の整形に失敗しました。時間をおいて再度お試しください。"
